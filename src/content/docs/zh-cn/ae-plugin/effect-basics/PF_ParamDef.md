...
After Effects 会通过每个选择器向效果传递一个 PF_ParamDefs 数组，描述插件在当前时间的参数。params 数组中的值仅在某些选择器期间有效（这在[选择器描述](command-selectors.md#calling-sequence)中有说明）。
...
| `PF_ChangeFlags`                            | `change_flags` | 如果您更改了参数值，则设置此标志。仅在拖动（而非点击！）事件、[PF_Cmd_USER_CHANGED_PARAM](command-selectors.md#messaging) 或 [PF_Cmd_UPDATE_PARAMS_UI](command-selectors.md#messaging) 期间有效。 |
...
| [PF_ParamType](parameters.md#parameter-types) | `param_type` | 参数类型。                                                                                                                                                                                             |
...
| `PF_ParamDefUnion`                          | `u`          | 所有可能的[参数类型](parameters.md#parameter-types)的联合体。                                                                                                                                             |
...
|                            | 此标志不需要设置 [PF_OutFlag_CUSTOM_UI](PF_OutData.md#pf_outflags) 标志。                                                                                                                                                                    |
|                            | 如果您想要 `PF_Param_ARBITRARY_DATA` 的标准控制，只需使用 `PF_PUI_STD_CONTROL_ONLY` 添加一个（或多个）支持参数类型的控件，然后在处理 [PF_Cmd_USER_CHANGED_PARAM](command-selectors.md#messaging) 时可以修改您的 arb 数据。 |
...
| `PF_PUI_DISABLED`          | 禁用（灰显）参数，通常是为了响应 [PF_Cmd_USER_CHANGED_PARAM](command-selectors.md#messaging)。                                                                                                                                               |
...
|                            | 仅限 Premiere：此标志是动态的，可以在 [PF_UpdateParamUI](../effect-details/parameter-supervision.md#pf_paramutilsuite3) 回调期间切换参数可见性。                                                                                     |

除了这些标志外，还可以使用 [AEGP_GetDynamicStreamFlags](../aegps/aegp-suites.md#aegp_dynamicstreamsuite4) 隐藏或显示效果参数。

---

## 参数标志

行为标志和 UI 标志描述了参数的不同特性。在 [PF_Cmd_PARAM_SETUP](command-selectors.md#global-selectors) 期间添加参数之前设置它们。在事件期间可以设置的标志已注明。
...
| `PF_ParamFlag_COLLAPSE_TWIRLY`                 | 在 [PF_Cmd_USER_CHANGED_PARAM](command-selectors.md#messaging) 期间设置此标志。                                                                                                                                                                                                                             |
|                                                | 现在可以在处理 [PF_Cmd_UPDATE_PARAMS_UI](command-selectors.md#messaging) 和 [PF_Cmd_USER_CHANGED_PARAM](command-selectors.md#messaging) 消息时设置和清除此位，以便随意旋转您的参数和组。                                                            |
| `PF_ParamFlag_SUPERVISE`                       | 设置以接收此参数的 [PF_Cmd_USER_CHANGED_PARAM](command-selectors.md#messaging) 消息。                                                                                                                                                                                                       |
|                                                | 有关更多信息，请参阅[参数监督](../../effect-details/parameter-supervision)。                                                                                                                                                                                                                 |
| `PF_ParamFlag_START_COLLAPSED`                 | 控制主题旋转器的旋转状态。                                                                                                                                                                                                                                                                  |
|                                                | 可以在参数监督期间更改，而不仅仅是在 [PF_Cmd_PARAM_SETUP](command-selectors.md#global-selectors) 期间。                                                                                                                                                                                     |
|                                                | 除非设置了 [PF_OutFlag2_PARAM_GROUP_START_COLLAPSED](PF_OutData.md#pf_outflags)，否则此标志不会被遵守。                                                                                                                                                                                             |
...
|                                                | 在 After Effects 中忽略。                                                                                                                                                                                                                                                                                     |
| `PF_ParamFlag_EXCLUDE_FROM_HAVE_INPUTS_CHANGED`| 仅当效果设置了 [PF_OutFlag2_AUTOMATIC_WIDE_TIME_INPUT](PF_OutData.md#pf_outflags) 并且将调用 [PF_AreStatesIdentical](../effect-details/parameter-supervision.md#pf_paramutilsuite3) 或 [PF_HaveInputsChangedOverTimeSpan](../effect-details/parameter-supervision.md#pf_paramutilsuite3) 时相关 |
