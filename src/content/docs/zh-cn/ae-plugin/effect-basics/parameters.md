...
在 [PF_Cmd_PARAM_SETUP](command-selectors.md#global-selectors) 期间使用 [PF_ADD_PARAM](../effect-details/interaction-callback-functions.md#interaction-callbacks) 描述插件的参数。
...
|                     |                           |                       |                | `PF_Param_FLOAT_SLIDER` 还响应 [音频滤波器](../audio/audio-considerations.md#audio-considerations) 中讨论的滑块标志。                                                                                                                                     |
...
|                     |                           |                       |                | 对于浮点精度，使用 [PF_ColorParamSuite1](../effect-details/parameters-floating-point-values.md#pf_colorparamsuite1) 来检索值。                                                                                                                      |
| `PF_Param_POINT`    | `PF_PointDef`              | `td`                  | `PF_Fixed`     | 二维点。该点提供目标层空间中的 x 和 y 值。                                                                                                                                                                                         |
|                     |                           |                       |                | 层的原点是左上角，x 向右增加，y 向下增加。                                                                                                                                                                      |
|                     |                           |                       |                | 从 CS5.5 开始，对于浮点精度，使用 [PF_PointParamSuite1](../effect-details/parameters-floating-point-values.md#pf_pointparamsuite1) 来检索值。                                                                                                   |
...
|                     |                           |                       |                | 路径参数数据不能直接访问；使用 [PF_PathQuerySuite1](../effect-details/working-with-paths.md#pf_pathquerysuite1) 和 [PF_PathDataSuite](../effect-details/working-with-paths.md#pf_pathdatasuite) 来管理和查询路径。                      |
|                     |                           |                       |                | `PF_PathDef.path_id` 包含用户选择的蒙版的索引。                                                                                                                                                                                                      |
|                     |                           |                       |                | 可以使用 [AEGP_MaskSuite6](../aegps/aegp-suites.md#aegp_masksuite6) 中的 `AEGP_GetLayerMaskByIndex` 获取相应的 `AEGP_MaskRefH`。                                                                                                                              |
